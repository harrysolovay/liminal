...
import { type Actor, type EventScope, Exec, L, type LEvent, type LiminalConfig } from "../index.ts"
...
export async function runExec(ctx: CliCtx, args: Array<string>) {
  ...
  const writeHandlerOrNoop = config.write
    ...
    : undefined
  const printHandlerOrNoop = config.silent ? undefined : (event: EventScope) => console.log(event)
